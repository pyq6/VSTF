...
class MAD(nn.Module):
    ...
    def __init__(self, c1, c2=None, dilation=2):
        super().__init__()
        if c2 is None:
            c2 = c1
        assert c1 == c2, "DAAR requires c1 == c2 for residual connection"

        self.c = c1
        
   
        self.branch1 = nn.Sequential(
            nn.Conv2d(c1, c1, kernel_size=(1, 3), padding=(0, 1), bias=False),
            nn.BatchNorm2d(c1),
        ...
        self.branch2 = nn.Sequential(
            nn.Conv2d(c1, c1, kernel_size=(1, 3), dilation=(1, dilation), padding=(0, dilation), bias=False),
            nn.BatchNorm2d(c1),
            nn.SiLU(inplace=True)
        )
        ...
        self.alpha_conv = nn.Sequential(
           nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(c1 * 2, c1, kernel_size=1, bias=True),
            nn.Sigmoid()
        ...
        self.channel_att = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(c1, c1, kernel_size=1, bias=True),
            nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        f_hor = self.fuse_hor(torch.cat([f1, f2], dim=1))
        
        
        alpha = self.alpha_conv(torch.cat([f_hor, f3], dim=1))
        f_space = alpha * f_hor + (1 - alpha) * f3

        
        w_ch = self.channel_att(f_space)
        out = f_space * w_ch
        
       
        return identity + self.res_scale * out

class VSEM(nn.Module):
    """
    VSEM: Visual Semantic-driven Environment Modulation
    ...
    def __init__(self, clip_model, c1, reduction=4):
        super().__init__()

        self.clip_model = clip_model
        self.clip_model.eval()  # 
       # self.clip_model = clip_model.eval().float()
      
        
        self.log_path = "vsem_log_exp4.csv"
        ...
        hidden_dim = 512 // reduction  

        self.mlp = nn.Sequential(
            nn.Linear(512, hidden_dim),
            nn.SiLU(inplace=True),
            nn.Dropout(0.15),
            nn.Linear(hidden_dim, 2 * c1)
        )
    ...
    def forward(self, x, img):
        ...
        img = img.to(device=x.device, dtype=torch.float32)
        
    
        # 2. CLIP
        with torch.no_grad():
                self.clip_model.float()
                z = self.clip_model.encode_image(img)
                z = z / (z.norm(dim=-1, keepdim=True) + 1e-6)
    
        z = z.to(self.mlp[0].weight.dtype)
        
    
        # 3. MLP
        gamma, beta = self.mlp(z).chunk(2, dim=1)
        gamma_act = self.scale * torch.tanh(gamma)
        beta_act  = self.scale * torch.tanh(beta)
        ...
        gamma = gamma_act.view(B, C, 1, 1)
        beta  = beta_act.view(B, C, 1, 1)
       
        return x * (1 + gamma.to(x.dtype)) + beta.to(x.dtype)
       
        
        
class P5_Enhancer(nn.Module):
...
class FAD(nn.Module):
    ...
    def __init__(self, c1, c2):
        ...
        hidden_dim = max(1, c1 // 4)
        self.freq_gate = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(c1 * 4, hidden_dim, 1, bias=False),
            nn.SiLU(),
            nn.Conv2d(hidden_dim, c1 * 4, 1, bias=False),
            nn.Sigmoid()
        )
        
        
        self.learnable_reconstruct = nn.Conv2d(c1 * 4, c1, 1, bias=False)
    ...
    def forward(self, x):
        ...
        freq_raw = self.dwt(x)
        freq_clean = freq_raw * self.freq_gate(freq_raw) 
        
     
        freq_feat = self.learnable_reconstruct(freq_clean)
        freq_feat = F.interpolate(freq_feat, size=x.shape[2:], mode='bilinear', align_corners=False)
        
       
        return self.fusion(torch.cat([spatial_feat, freq_feat], dim=1))
